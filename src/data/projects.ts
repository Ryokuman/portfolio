...
export const projects: Project[] = [
  ...
  },
  {
    ...
    details: [
      ...
      {
        title: "Storybook + Mock Backend 독립 개발 환경",
        content: [
          "문제: 프론트 개발이 백엔드 API 완성까지 대기 — 프로젝트 일정 병목",
          "Storybook Vite plugin으로 /agent/* 요청을 Next.js dev 서버로 프록시, 개별 mock route에서 mockdata 주입하는 2단계 개발 환경 구축",
          "프론트가 API 스펙(요청/응답 형태)을 먼저 정의하고 백엔드에 전달 — 프론트 리드로서 API 설계 의사결정 주도, 백엔드도 스펙만 맞추면 되어 양쪽 개발 효율화",
          "Storybook 24개 story로 코어 컴포넌트 사용법 문서화 → 팀원 온보딩 시 코드를 직접 실행하며 학습 가능",
          "백엔드 대기 pending 완전 제거 — PR 속도 2.8배 추가 향상 (2.3/일 → 6.4/일)",
        ],
